...
int main () {
	freopen("result_num2(1).txt", "w", stdout);
	for (int n = 3; n <= 21; ++n) for (int k = 1; k + 1 <= n; ++k) for (int l = 1; l + k <= n; ++l) {
    double tt = clock();
	// calculate
	...
	vector <int> ans(m);
	for (int i = 0; i < m; ++i)
		ans[i] = possible_mask[getRand(0, possible_mask.size() - 1)];
